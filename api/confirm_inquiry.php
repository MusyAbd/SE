<?php
// FILE: api/confirm_inquiry.php (FINAL)
...
$sql = "UPDATE sales_inquiries SET status = 'accepted', updatedAt = ? WHERE id = ? AND status = 'active'";
...
if ($stmt) {
    ...
    if (mysqli_stmt_execute($stmt)) {
        if (mysqli_stmt_affected_rows($stmt) > 0) {
            ...
            echo json_encode(['success' => true, 'message' => 'Inquiry has been confirmed successfully.']);
        } else {
            echo json_encode(['success' => false, 'message' => 'Inquiry not found or was already confirmed.']);
        }
    } else {
